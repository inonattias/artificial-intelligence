...
def calc_ham_sum_of_path(lst: List[str], graph_dict: Dict[str, Dict[str, float]]) -> float:
    sum: float = 0.0
...
def calc_sum_of_path(lst: List[str], graph_dict: Dict[str, Dict[str, float]]) -> float:
    sum: float = 0.0
    for index, node in enumerate(lst):
        if index == len(lst) - 1:
...
def _calc_initial(graph: Graph) -> Tuple[List[str], float]:
    ...
    return initial_list, calc_sum_of_path(initial_list, graph.graph_dict)
...
class TSPAnnealingProblem:
    ...
    def get_random_next_state(self, current_state: Tuple[List[str], float]) -> Tuple[List[str], float]:
        ...
        assert int(my_sum) == int(calc_sum_of_path(new_path, self.graph)), "calc_sum:" + str(
            my_sum) + "True sum:" + str(_calc_sum_of_path(new_path, self.graph))
        return new_path, my_sum

    def calc_hamilton_path_len(self, current_state) -> float:
        cur_path, cur_sum = current_state
        return cur_sum - self.graph[cur_path[-1]][cur_path[1]]
